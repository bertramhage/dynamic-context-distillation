...
### Job Name:
#BSUB -J train_only_new_02_14d_t_v2
...
uv run python -m src.training.main \
  ...
  training.length_jitter.long_min_steps=3024 \
  training.length_jitter.long_max_steps=5040 \
  training.length_jitter.short_min_steps=144 \
  training.length_jitter.short_max_steps=432 \
  wandb.enabled=true \
  training_loop.target_mode=teacher \
  training_loop.checkpoint_dir=checkpoints/final_14d_t_v2 \
  "wandb.run_name=${TRAIN_RUN_NAME}" \
  "wandb.tags=[hpc,fullscale,training,v2,teacher]"
