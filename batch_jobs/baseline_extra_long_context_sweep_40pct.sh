...
#BSUB -o batch_jobs/logs/baseline_ctx_max_28d_%J.out
#BSUB -e batch_jobs/logs/baseline_ctx_max_28d_%J.err
...
RUN_PREFIX="baseline_maxctx28d_subset40_${LSB_JOBID:-$(date +%Y%m%d_%H%M%S)}"
...
for cl in true false; do
  ...
  run_name="${RUN_PREFIX}_ctx_28d_${cl_label}"

  echo "Running baseline max-context eval: 28d (8064 steps), cross_learning=${cl}"
  ...
  uv run python -m src.evaluation.main \
    ...
    cross_learning="${cl}" \
    seed="${BASE_SEED}" \
    evaluation.history_length_steps=8064 \
...
echo "Max-context baseline job finished successfully."
